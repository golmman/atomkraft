...
Endgames::Endgames() {
  ...
  OLD add<Endgame<Value, KRKP>  >("KRKP");
  add<Endgame<Value, KRKB>  >("KRKB");
  add<Endgame<Value, KRKN>  >("KRKN");
  OLD add<Endgame<Value, KQKR>  >("KQKR");
  ...
  STARTNEW
  add<Endgame<Value, KQK_ATOMIC>			>("KQK");
  add<Endgame<Value, KRK_ATOMIC>			>("KRK");
  add<Endgame<Value, KRRK_ATOMIC>			>("KRRK");
  add<Endgame<Value, KRBK_ATOMIC>			>("KRBK");
  add<Endgame<Value, KRNK_ATOMIC>			>("KRNK");
  add<Endgame<Value, KQKP_ATOMIC>			>("KQKP");
  add<Endgame<Value, KQPKP_ATOMIC>			>("KQPKP");
  add<Endgame<Value, KRPKP_ATOMIC>			>("KRPKP");
  ENDNEW
}
...
template<>
Value Endgame<Value, KQK_ATOMIC>::apply(const Position& pos) const {
	
	assert(pos.piece_count(WHITE, KING) && pos.piece_count(BLACK, KING));
	assert(pos.piece_count(strongerSide, QUEEN) == 1);
	assert(square_is_ok(pos.piece_list(strongerSide, QUEEN, 0)));
	
	Square strong_ksq = pos.king_square(strongerSide);
	Square weak_ksq = pos.king_square(weakerSide);
	
	// easy cases
	if (    square_dist(strong_ksq, weak_ksq) < 2 
		|| (square_dist(strong_ksq, weak_ksq) == 2 && pos.side_to_move() == weakerSide)) {
		// kings are adjacent or the weaker side is able to make kings adjacent immediatly
		return VALUE_DRAW;
	} else if (    square_dist(strong_ksq, weak_ksq) > 3
			   || (square_dist(strong_ksq, weak_ksq) == 3 && pos.side_to_move() == strongerSide)) {
		// kings are not adjacent and the stronger side can prevent this
		return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
	} else {
		// TODO: all the other cases are too difficult
		// maybe i should create some "endgame table bases"
		
//		// now the problematic cases:
//		// 1. kings have distance 2 and the stronger side moves
//		// 2. kings have distance 3 and the weaker side moves
//		
//		Square qsq = pos.piece_list(strongerSide, QUEEN, 0);
//		Rank qr = square_rank(qsq);
//		File qf = square_file(qsq);
//		
//		Rank strong_kr = square_rank(strong_ksq);
//		File strong_kf = square_file(strong_ksq);
//		Rank weak_kr = square_rank(weak_ksq);
//		File weak_kf = square_file(weak_ksq);
//		
//		assert(qr != weak_kr && qf != weak_kf);
//		
//		// the queens rank and file (non diagonal) attacks separate the board
//		// into 4 quadrants and 4 axis, only the strong sides king can be on an axis
//		//			A2
//		//		Q2  |  Q1
//		//	 A3-----+-----A1
//		//		Q3  |  Q4
//		//			A4
//		enum QueenQuadrant {
//			QUAD1, QUAD2, QUAD3, QUAD4, AXIS1, AXIS2, AXIS3, AXIS4
//		};
//		
//		QueenQuadrant strong_kqu;
//		QueenQuadrant weak_kqu;
//		
//		// determine weak side kings quadrant
//		if (weak_kr > qr) {
//			if (weak_kf > qf) 	weak_kqu = QUAD1;
//			else 				weak_kqu = QUAD2;
//		} else {
//			if (weak_kf < qf) 	weak_kqu = QUAD3;
//			else 				weak_kqu = QUAD4;
//		}
//		
//		// determine strong side kings quadrant 
//		if (strong_kr == qr) {
//			if     (strong_kf > qf) 	strong_kqu = AXIS1;
//			else /*(strong_kf < qf)*/	strong_kqu = AXIS3;
//		} else if (strong_kr > qr) {
//			if      (strong_kf == qf) 	strong_kqu = AXIS2;
//			else if (strong_kf > qf)	strong_kqu = QUAD1;
//			else  /*(strong_kf < qf)*/	strong_kqu = QUAD2;
//		} else {
//			if      (strong_kf == qf) 	strong_kqu = AXIS4;
//			else if (strong_kf < qf) 	strong_kqu = QUAD3;
//			else  /*(strong_kf > qf)*/	strong_kqu = QUAD4;
//		}
//		
//		
//		
//		if (strong_kqu >= AXIS1) {
//			// stronger sides king stands onto an axis
//			// this is always a special case of strong_kqu != weak_kqu or the last "else"
//			
//		} else if (abs(strong_kqu - weak_kqu) == 2) {
//			// kings are in diagonal facing quadrants
//			return VALUE_KNOWN_WIN;
//			
//		} else if (strong_kqu != weak_kqu) {
//			// TODO: NOT TRUE!
//			// kings are on neighboring quadrants, i.e. strong side needs space to "escape"
//			if 		  (strong_kqu == QUAD1 && weak_kqu == QUAD4) {
//				if (qr <= RANK_6) return VALUE_KNOWN_WIN;	//----------------
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD1 && weak_kqu == QUAD2) {
//				if (qf <= FILE_F) return VALUE_KNOWN_WIN;	//++++++++++++++++
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD2 && weak_kqu == QUAD1) {
//				if (qf >= FILE_C) return VALUE_KNOWN_WIN;	//~~~~~~~~~~~~~~~~
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD2 && weak_kqu == QUAD3) {
//				if (qr <= RANK_6) return VALUE_KNOWN_WIN;	//----------------
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD3 && weak_kqu == QUAD2) {
//				if (qr >= RANK_3) return VALUE_KNOWN_WIN;	//****************
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD3 && weak_kqu == QUAD4) {
//				if (qf >= FILE_C) return VALUE_KNOWN_WIN;	//~~~~~~~~~~~~~~~~
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD4 && weak_kqu == QUAD3) {
//				if (qf <= FILE_F) return VALUE_KNOWN_WIN;	//++++++++++++++++
//				else 			  return VALUE_DRAW;
//			} else if (strong_kqu == QUAD4 && weak_kqu == QUAD1) {
//				if (qr >= RANK_3) return VALUE_KNOWN_WIN;	//****************
//				else 			  return VALUE_DRAW;
//			} else {
//				assert(0);
...
//			
//		} else {
//			// both kings are in the same quadrant
//			
//		}
	}
	return VALUE_NONE;
...
template<>
Value Endgame<Value, KRK_ATOMIC>::apply(const Position& pos) const {
	return VALUE_DRAW;
}

template<>
Value Endgame<Value, KRRK_ATOMIC>::apply(const Position& pos) const {
	return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
}

template<>
Value Endgame<Value, KRBK_ATOMIC>::apply(const Position& pos) const {
	return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
}

template<>
Value Endgame<Value, KRNK_ATOMIC>::apply(const Position& pos) const {
	return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
}

template<>
Value Endgame<Value, KQKP_ATOMIC>::apply(const Position& pos) const {
...
template<>
Value Endgame<Value, KQPKP_ATOMIC>::apply(const Position& pos) const {
	return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
}

template<>
Value Endgame<Value, KRPKP_ATOMIC>::apply(const Position& pos) const {
	// there are 6 draw pawn configurations all the others are a win for white
	
#define S(x) (1ULL << (x))
	static const Bitboard draw_pattern_white = 
		S(SQ_A3) | S(SQ_E3) | S(SQ_D3) | S(SQ_H3) | S(SQ_E4) | S(SQ_D4);
	static const Bitboard draw_pattern_black = 
		S(SQ_A6) | S(SQ_E6) | S(SQ_D6) | S(SQ_H6) | S(SQ_E5) | S(SQ_D5);
#undef S
	
	if (strongerSide == WHITE) {
		Bitboard blocked_pawn = pos.pieces(PAWN, WHITE) & draw_pattern_white;
		if ((blocked_pawn << 8) & pos.pieces(PAWN, BLACK)) {
			return VALUE_DRAW;
		}
	} else {
		Bitboard blocked_pawn = pos.pieces(PAWN, BLACK) & draw_pattern_black;
		if ((blocked_pawn >> 8) & pos.pieces(PAWN, WHITE)) {
			return VALUE_DRAW;
		}
	}
...
// TODO: not used yet
template<>
Value Endgame<Value, KQXsK_ATOMIC>::apply(const Position& pos) const {
	return VALUE_KNOWN_WIN;
}

template<>
Value Endgame<Value, KPsKPs_ATOMIC>::apply(const Position& pos) const {
	return VALUE_DRAW;
}

template<>
Value Endgame<Value, KXKP_ATOMIC>::apply(const Position& pos) const {
	return VALUE_DRAW;
}

template<>
Value Endgame<Value, PAWNITIZATION_ATOMIC>::apply(const Position& pos) const {
	return VALUE_DRAW;
}
...
Value Endgame<Value, KXK>::apply(const Position& pos) const {
  ...
  if (    pos.piece_count(strongerSide, QUEEN) 
	  && (   pos.piece_count(strongerSide, ROOK)
		  || pos.piece_count(strongerSide, BISHOP)
		  || pos.piece_count(strongerSide, KNIGHT)
	)) {
	  return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
  }
  
  if (pos.piece_count(strongerSide, PAWN) > 1) {
	  return strongerSide == pos.side_to_move() ? VALUE_KNOWN_WIN : -VALUE_KNOWN_WIN;
  }
...
Value Endgame<Value, KPK>::apply(const Position& pos) const {
  ...
  STARTNEW
  Square spsq = pos.piece_list(strongerSide, PAWN, 0);	// stronger pawn square
  wksq = pos.king_square(WHITE);
  bksq = pos.king_square(BLACK);
  stm = pos.side_to_move();
  
  int moves_to_adjacent_kings 	= square_dist(wksq, bksq) - 1;
  int moves_to_promote 			= RANK_8 - relative_rank(strongerSide, spsq);
  
  // we can evaluate the outcome of some patterns, similar to KQK
  if (strongerSide == stm) {
	  if 		(moves_to_promote + 2 <= moves_to_adjacent_kings) {
		  return VALUE_KNOWN_WIN;
	  } else if (moves_to_promote >= moves_to_adjacent_kings) {
		  return VALUE_DRAW;
	  }
  } else {
	  // if the weaker side moves the distances can be even bigger
	  if 		(moves_to_promote + 3 <= moves_to_adjacent_kings) {
		  return -VALUE_KNOWN_WIN;
	  } else if (moves_to_promote + 1 >= moves_to_adjacent_kings) {
		  return VALUE_DRAW;
	  }
  }
