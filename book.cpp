...
NEW // longer longer inside namespace
...
Book::Book() {
  ...
  OLD for (int i = abs(get_system_time() % 10000); i > 0; i--)
      RKiss.rand<unsigned>();
  
  STARTNEW
  // in our implementation get_system_time returns the milliseconds since program start 
  for (int i = abs(rand() % 10000); i > 0; i--)
        RKiss.rand<unsigned>();
  ENDNEW
}
...
void Book::open(const string& fileName) {
  ...
  OLD bookFile.open(fileName.c_str(), ifstream::in | ifstream::binary);
  NEW bookFile.open(fileName.c_str(), 
		  ifstream::in | ifstream::binary | ofstream::binary | ofstream::out);
...
Move Book::get_move(const Position& pos, bool findBestMove) {
  ...
  for (int idx = find_entry(key); idx < bookSize; idx++)
  {
      ...
      NEW // skip moves with non positive score
      NEW if (score <= 0) continue;
...
STARTNEW
template<typename T>
void Book::write_number(T& n) {


  for (size_t i = 0; i < sizeof(T); i++) {
	  char c = char((n >> ((sizeof(T) - 1) * 8)));
	  n <<= 8;
	  bookFile.write(&c, 1);
  }
...
void Book::write_entry(uint64_t key, Move move, uint16_t count) {
...
void Book::print_all_moves(const Position& pos) {
	  if (!bookFile.is_open() || bookSize == 0) {
	  ...
	  for (int idx = find_entry(key); idx < bookSize; idx++)
	  {
	      ...
	      cout << "(" << move_to_string(Move(entry.move)) << " " << entry.count << ") ";
	  }
	  cout << endl;
